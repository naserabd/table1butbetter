---
title: "Using table1butbetter with \\LaTeX{} and MS Word"
...
header-includes:
  ...
  - \usepackage{xcolor}
vignette: >
  %\VignetteIndexEntry{Using table1butbetter with \LaTex{}}
...
A long requested feature has been the ability to use `table1butbetter` in `rmarkdown`
documents that render to \LaTeX{} or `.docx` (i.e.
Microsoft\textsuperscript{\textregistered} Word).  Since version 1.4 of
`table1butbetter`, this is now possible (with some limitations) by converting the output
of `table1butbetter()` to a `data.frame`, `kableExtra` or `flextable`, using the
...
```{r}
x <- table1butbetter(~ age + sex + wt | treat, data=dat)
as.data.frame(x)
```
...
```{r}
x2 <- table1butbetter(~ age + wt | treat*sex, data=dat, overall=FALSE)
t1kable(x2)
```
...
As of version 1.4, it is now possible to use `table1butbetter` within \LaTeX{} documents.
